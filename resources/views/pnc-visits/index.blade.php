<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-black dark:text-white">Kunjungan Nifas (PNC)</h2>
        <a href="{{ route('pnc-visits.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Catat Kunjungan
        </a>
    </div>
            ...
            <table id="dataTable" class="w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-left dark:bg-gray-700">
                        ...
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Kode Kunjungan</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Tensi</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Kondisi</th>
                        ...
                        <tr>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">
                                    {{ $v->appointment->appointment_date->format('d M Y') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <h5 class="font-medium text-black dark:text-white">{{ $v->delivery->patient->name }}</h5>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <span
                            ...
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-sm font-mono">{{ $v->blood_pressure }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-xs">Lokia: {{ $v->lochia_condition }}</p>
                                <p class="text-xs">Uterus: {{ $v->uterine_involution }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('pnc-visits.edit', $v) }}" class="text-primary hover:underline"><i
                                            class="fas fa-edit"></i></a>
                                    <form action="{{ route('pnc-visits.destroy', $v) }}" method="POST"
