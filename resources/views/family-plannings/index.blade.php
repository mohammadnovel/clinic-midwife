<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-black dark:text-white">Keluarga Berencana (KB)</h2>
        <a href="{{ route('family-plannings.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Kunjungan KB Baru
        </a>
    </div>
            ...
            <table id="dataTable" class="w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-left dark:bg-gray-700">
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Tanggal Pasang / Kunjungan</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Pasien</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Metode KB</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Jadwal Ulang</th>
                        ...
                        <tr>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">
                                    {{ \Carbon\Carbon::parse($rec->installation_date)->format('d M Y') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                            ...
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p
                                    class="text-sm font-mono {{ \Carbon\Carbon::parse($rec->next_visit_date)->isPast() ? 'text-red-500' : 'text-green-500' }}">
                                    {{ $rec->next_visit_date ? \Carbon\Carbon::parse($rec->next_visit_date)->format('d M Y') : '-' }}
                                </p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('family-plannings.edit', $rec) }}"
                                        class="text-primary hover:underline"><i class="fas fa-edit"></i></a>
                                    <form action="{{ route('family-plannings.destroy', $rec) }}" method="POST"
                                        onsubmit="return confirm('Hapus data KB ini?');">
                                        @csrf
                                        @method('DELETE')
