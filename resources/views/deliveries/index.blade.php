<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-black dark:text-white">Riwayat Persalinan (Partus)</h2>
        <a href="{{ route('deliveries.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Input Persalinan
        </a>
    </div>
            ...
            <table id="dataTable" class="w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-left dark:bg-gray-700">
                        ...
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Bayi</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Kondisi Lahir</th>
                    ...
                    @foreach($deliveries as $d)
                        <tr>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $d->delivery_time->format('d M Y H:i') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <h5 class="font-medium text-black dark:text-white">{{ $d->pregnancy->patient->name }}</h5>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <span
                            ...
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                @if($d->baby)
                                    <p class="text-sm">Nama: {{ $d->baby->name }}</p>
                                    <p class="text-xs text-gray-500">
                                        {{ $d->baby->gender == 'male' ? 'Laki-laki' : 'Perempuan' }} |
                                        {{ $d->baby->birth_weight }} kg</p>
                                @else
                                    <span class="text-xs text-red-500">Belum didata</span>
                                @endif
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-sm">{{ $d->birth_condition }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('deliveries.edit', $d) }}" class="text-primary hover:underline"><i
                                            class="fas fa-edit"></i></a>
                                    <form action="{{ route('deliveries.destroy', $d) }}" method="POST"
