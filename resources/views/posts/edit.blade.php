...
<div class="mx-auto max-w-3xl">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-2xl font-bold text-black dark:text-white"><i class="fas fa-edit mr-2 text-primary"></i>Edit Artikel</h2>
        <div class="flex gap-2">
            <a href="{{ route('posts.show', $post->id) }}" class="inline-flex items-center gap-2 rounded-lg border border-stroke bg-white px-4 py-2 text-sm font-medium text-black hover:bg-gray-50 dark:bg-meta-4 dark:text-white dark:border-strokedark shadow-sm">
    ...
    <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="rounded-xl border border-stroke bg-white shadow-sm dark:border-strokedark dark:bg-boxdark">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $post->title) }}" required placeholder="Judul artikel..."
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="2" placeholder="Ringkasan singkat artikel..."
                        class="w-full rounded-lg border border-stroke bg-transparent py-3 px-4 text-sm outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input">{{ old('excerpt', $post->excerpt) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-stroke bg-white shadow-sm dark:border-strokedark dark:bg-boxdark">
            ...
            <div class="p-6">
                <textarea name="content" rows="14" required placeholder="Tulis konten artikel di sini..."
                    class="w-full rounded-lg border border-stroke bg-transparent py-3 px-4 text-sm outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input font-mono">{{ old('content', $post->content) }}</textarea>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-stroke bg-white shadow-sm dark:border-strokedark dark:bg-boxdark">
            ...
            <div class="p-6">
                @if($post->thumbnail)
                <div class="mb-4" id="current-thumb">
                    <p class="text-xs text-gray-400 mb-2">Thumbnail saat ini:</p>
                    <img src="{{ asset('storage/' . $post->thumbnail) }}" class="h-40 rounded-xl object-cover">
                ...
                <div id="preview-wrap" class="mt-3 hidden">
                    ...
                    <button type="button" onclick="clearThumb()" class="mt-2 text-xs text-red-400 hover:text-red-600"><i class="fas fa-times mr-1"></i>Hapus pilihan</button>
                </div>
            </div>
        </div>
