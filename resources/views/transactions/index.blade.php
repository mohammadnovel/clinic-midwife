<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-black dark:text-white">Data Transaksi & Pembayaran</h2>
            ...
            <table id="dataTable" class="w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-left dark:bg-gray-700">
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Kode Invoice</th>
                        ...
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Total Tagihan</th>
                    ...
                    @foreach($transactions as $trx)
                        <tr>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="font-mono font-medium text-primary">{{ $trx->code }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $trx->created_at->format('d/m/Y') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <h5 class="font-medium text-black dark:text-white">{{ $trx->patient->name }}</h5>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white font-bold">Rp
                                    {{ number_format($trx->total_amount, 0, ',', '.') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <span
                                    class="inline-flex rounded-full {{ $trx->payment_status == 'paid' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }} py-1 px-3 text-sm font-medium">
                                    {{ ucfirst($trx->payment_status) }}
                                </span>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('transactions.edit', $trx) }}" class="text-primary hover:underline"><i
