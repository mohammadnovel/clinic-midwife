...
<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-black dark:text-white"><i class="fas fa-hand-holding-medical mr-2 text-primary"></i>Tambah Layanan</h2>
            <p class="text-sm text-gray-500 mt-1">Tambahkan layanan / tindakan klinik baru.</p>
        </div>
        ...
        <form action="{{ route('services.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" placeholder="Contoh: ANC01" required
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Layanan" required
                        class="w-full rounded-lg border border-stroke bg-transparent py-3 px-4 text-sm outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border border-stroke p-3 text-sm transition has-[:checked]:border-green-500 has-[:checked]:bg-green-50 dark:border-strokedark dark:has-[:checked]:bg-green-900/20">
                        <input type="radio" name="is_active" value="1" checked class="accent-green-500" /> <span class="font-medium">✅ Aktif</span>
                    </label>
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border border-stroke p-3 text-sm transition has-[:checked]:border-red-400 has-[:checked]:bg-red-50 dark:border-strokedark dark:has-[:checked]:bg-red-900/20">
                        <input type="radio" name="is_active" value="0" class="accent-red-500" /> <span class="font-medium text-red-600">⛔ Non-aktif</span>
                    </label>
                </div>
            </div>
