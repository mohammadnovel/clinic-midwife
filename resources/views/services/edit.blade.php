<x-admin-layout>
    <div class="mx-auto max-w-270">
        <div class="mb-6 flex gap-3 sm:items-center">
            ...
            <h2 class="text-title-md2 font-bold text-black dark:text-white">Edit Data Layanan</h2>
        </div>

        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <form action="{{ route('services.update', $service->id) }}" method="POST" class="p-6.5">
                @csrf
                @method('PUT')
                <div class="mb-4.5">
                ...
                <div class="mb-4.5">
                    ...
                    <div class="relative z-20 bg-transparent dark:bg-form-input">
                        <select name="category"
                            ...
                            @foreach(['General', 'ANC', 'Delivery', 'Immunization', 'KB'] as $cat)
                                <option value="{{ $cat }}" {{ $service->category == $cat ? 'selected' : '' }}>{{ $cat }}
                                </option>
                            @endforeach
                        </select>
                    </div>
