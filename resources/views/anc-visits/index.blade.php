<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-black dark:text-white">Pemeriksaan Kehamilan (ANC)</h2>
        <a href="{{ route('anc-visits.create') }}"
            ...
            <i class="fas fa-plus mr-2"></i> Tambah Kunjungan ANC
        </a>
    </div>
            ...
            <table id="dataTable" class="w-full table-auto display">
                <thead>
                    <tr class="bg-gray-200 text-left dark:bg-gray-700">
                        ...
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Usia Kehamilan</th>
                        <th class="py-4 px-4 font-medium text-black dark:text-white">Keluhan</th>
                    ...
                    @foreach($visits as $visit)
                        <tr>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $visit->created_at->format('d/m/Y H:i') }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <h5 class="font-medium text-black dark:text-white">{{ $visit->pregnancy->patient->name }}
                                </h5>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-black dark:text-white">{{ $visit->gestational_age_weeks }} Minggu</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-sm text-gray-500">{{ Str::limit($visit->complaints, 30) }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <p class="text-sm text-gray-500">{{ Str::limit($visit->actions, 30) }}</p>
                            </td>
                            <td class="border-b border-gray-200 py-5 px-4 dark:border-gray-700">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('anc-visits.edit', $visit) }}" class="text-primary hover:underline"><i
                                            class="fas fa-edit"></i></a>
                                    <form action="{{ route('anc-visits.destroy', $visit) }}" method="POST"
                                        onsubmit="return confirm('Hapus data ANC ini?');">
                                        @csrf
                                        @method('DELETE')
