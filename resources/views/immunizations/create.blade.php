...
<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-black dark:text-white"><i class="fas fa-syringe mr-2 text-green-500"></i>Catat Vaksinasi / Imunisasi</h2>
            <p class="text-sm text-gray-500 mt-1">Pencatatan pemberian vaksin kepada pasien.</p>
        </div>
    ...
    <div class="rounded-xl border border-stroke bg-white shadow-sm dark:border-strokedark dark:bg-boxdark">
        ...
        <form action="{{ route('immunizations.store') }}" method="POST" class="p-6 space-y-4">
            @csrf

            <div>
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Pasien / Anak <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Jenis Vaksin <span class="text-red-500">*</span></label>
                <select name="immunization_type_id" required class="w-full rounded-lg border border-stroke bg-transparent py-3 px-4 text-sm outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    <option value="">-- Pilih Jenis Vaksin --</option>
                    @foreach($types as $t)
                    <option value="{{ $t->id }}" {{ old('immunization_type_id') == $t->id ? 'selected' : '' }}>
                        {{ $t->name }} &mdash; Usia: {{ $t->recommended_age_months }} bulan
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="mb-2 block text-sm font-medium text-black dark:text-white">Tanggal Vaksinasi <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <input type="text" name="batch_number" value="{{ old('batch_number') }}" placeholder="Contoh: BATCH-12345"
                        class="w-full rounded-lg border border-stroke bg-transparent py-3 px-4 text-sm outline-none focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                </div>
            </div>

            <div>
                <label class="mb-2 block text-sm font-medium text-black dark:text-white">Catatan / Reaksi Vaksin</label>
                <textarea name="notes" rows="3" placeholder="Reaksi setelah vaksinasi, efek samping, dll..."
            ...
            <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3.5 text-sm font-semibold text-white transition hover:bg-opacity-90 active:scale-95">
                <i class="fas fa-save"></i> Simpan Data Imunisasi
            </button>
        </form>
    </div>
</div>
